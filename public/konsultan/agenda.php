...
$sql = "SELECT konsultasi.*, umkm.nama AS nama_umkm, umkm.foto_profil AS foto_umkm 
        FROM konsultasi 
        INNER JOIN umkm ON konsultasi.id_umkm = umkm.id_umkm 
        WHERE konsultasi.id_konsultan = $id_konsultan";

$result = $conn->query($sql);

if ($result) {
    $daftarKonsultasi = array();

    while ($row = $result->fetch_assoc()) {
        $daftarKonsultasi[] = $row;
    }
    ...
    foreach ($daftarKonsultasi as $konsultasi) {
        ...
        <div class="mt-8">
            ...
            <div class="flex justify-between">
                <div class="my-2">
                    <p class="font-semibold text-base mb-2">Client</p>
                    <div class="flex space-x-2">
                        <img src="../assets/img/profil/' . $konsultasi['foto_umkm'] . '" class="w-6 h-6 rounded-full" />
                        <span>' . $konsultasi['nama_umkm'] . '</span>
                    </div>
                </div>
                <div class="my-2">
